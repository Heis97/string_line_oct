...
bool GCodeQueue::process_injected_command_P() {
  ...
  if (i) {
    parser.parse(cmd);
    gcode.process_parsed_command(cmd);
  }
...
bool GCodeQueue::process_injected_command() {
  ...
  if (i) {
    ...
    gcode.process_parsed_command(injected_commands);
  }
...
//simple
 void GCodeQueue::get_serial_commands_orig() {
  #if ENABLED(BINARY_FILE_TRANSFER)
    ...
    for (uint8_t p = 0; p < NUM_SERIAL; ++p) {
      ...
      if (ISEOL(serial_char)) {
        ...
        if (npos) {
          ...
          /*f (gcode_N != serial.last_N + 1 && !M110) {
...
int GCodeQueue::get_serial_commands(char* line,byte len) {
        ...
        if (npos) {
          ...
          if (gcode_N != last_N + 1) {
            // A request-for-resend line was already in transit so we got two - oops!
            if (WITHIN(gcode_N, last_N - 1, last_N)) 
            {
// A corrupted line or too high, indicating a lost line
