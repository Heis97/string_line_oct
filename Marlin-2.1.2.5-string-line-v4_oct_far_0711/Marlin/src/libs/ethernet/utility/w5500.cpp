...
SoftSPIB soft_spi(SPI_SOFT_MOSI,SPI_SOFT_MISO,SPI_SOFT_SCK);
uint8_t SPI_CS;
...
void W5500Class::init_soft_spi(uint8_t cs_pin)//,uint8_t mosi, uint8_t miso, uint8_t sck
{
  delay(1000);
   SPI_CS = cs_pin;
  initSS();
  ...
  soft_spi.begin();
  soft_spi.setBitOrder(MSBFIRST);
  soft_spi.setDataMode(SPI_MODE2);
  w5500.swReset();
  for (int i=0; i<MAX_SOCK_NUM; i++) 
  {
    uint8_t cntl_byte = (0x0C + (i<<5));
...
{
    setSS();  
    soft_spi.spiSend(_addr >> 8);//  soft_spi.transfer(_addr >> 8);
    soft_spi.spiSend(_addr & 0xFF); // soft_spi.transfer(_addr & 0xFF);
    soft_spi.spiSend (_cb);// soft_spi.transfer(_cb);
    soft_spi.spiSend(_data);
    resetSS();
    return 1;
}
...
uint16_t W5500Class::write(uint16_t _addr, uint8_t _cb, const uint8_t *_buf, uint16_t _len)
{
    ...
    for (uint16_t i=0; i<_len; i++){
        soft_spi.transfer(_buf[i]);
    }

    //soft_spi.spiRead(_buf,_len);
    resetSS();
    return _len;
}
